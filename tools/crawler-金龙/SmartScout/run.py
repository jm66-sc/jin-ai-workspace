...
def check_requirements():
    ...
    for package in packages:
        ...
        import_mapping = {
            ...
            'pyyaml': 'yaml',
            'python-jose': 'jose',  # python-jose[cryptography] -> jose
            'passlib': 'passlib',   # passlib[bcrypt] -> passlib
        }
    ...
    for config_file in config_files:
        if os.path.exists(config_file):
            ...
            if config_file.endswith('secrets.yaml'):
                ...
                print("    复制命令: cp config/secrets.yaml.template config/secrets.yaml")
                print("    然后编辑 config/secrets.yaml 填写您的DeepSeek API密钥和其他敏感配置")
...
def main():
    ...
    try:
        ...
        # 检查是否启用测试模式
        test_mode = os.environ.get('SMARTSCOUT_TEST_MODE', '').lower() in ('1', 'true', 'yes', 'y')
        if test_mode:
            print("\n" + "="*60)
            print("测试模式已启用 - 跳过服务器启动")
            print("所有启动前检查已通过 ✓")
            print("="*60)
            print("\n验证完成: 系统配置正确，可以正常启动")
            return  # 正常退出，不启动服务器
